...
def doc_to_text(file_path):
    ...
    result = subprocess.run(['antiword', 'temp.doc'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
with tab1:
    ...
    uploaded_files = st.file_uploader(
        '**Choose your resume.pdf file:** ', type=["pdf", "docx", "doc"], accept_multiple_files=True)
    ...
    if comp_pressed and uploaded_files and JD_file:
        with st.spinner('Comparing resumes with job description...'):
            ...
            for file in uploaded_files:
                ...
                elif file.name.endswith(".doc") or file.name.endswith(".DOC"):
                    uploaded_file_paths.append(doc_to_text(file))
